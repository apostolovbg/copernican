...
MODEL_DESCRIPTION = "Standard flat Lambda Cold Dark Matter model."
...
# ... (rest of the file is correct and unchanged from the previous complete version) ...
...
def _hubble_parameter_normalized_sq(z, Omega_m0, Omega_r0, Omega_L0, Omega_k0=0):
    one_plus_z = 1 + z
    return Omega_r0 * one_plus_z**4 + Omega_m0 * one_plus_z**3 + Omega_k0 * one_plus_z**2 + Omega_L0
...
def get_Hz_per_Mpc(z_array, H0, Omega_m0, Omega_b0):
    z_array = np.asarray(z_array); h, Omega_r0, Omega_L0, Omega_k0, _ = _get_derived_densities(H0, Omega_m0, Omega_b0)
    if np.isnan(h): return np.full_like(z_array, np.nan, dtype=float)
    Ez_sq = _hubble_parameter_normalized_sq(z_array, Omega_m0, Omega_r0, Omega_L0, Omega_k0)
...
def get_comoving_distance_Mpc(z_array, H0, Omega_m0, Omega_b0):
    # This function should be optimized like the USMF one for performance.
    # For now, keeping the simpler loop as it's less of a bottleneck than USMF's.
    z_array_np = np.asarray(z_array); results_Mpc = np.full_like(z_array_np, np.nan, dtype=float)
    h_test, _, _, _, _ = _get_derived_densities(H0, Omega_m0, Omega_b0)
    if np.isnan(h_test): return results_Mpc.item() if z_array_np.ndim == 0 else results_Mpc
    for i, zi in np.ndenumerate(z_array_np):
        if abs(float(zi)) < 1e-9: results_Mpc[i] = 0.0; continue
        try:
            Dc_val, err = quad(_integrand_Dc, 0, float(zi), args=(H0, Omega_m0, Omega_b0))
...
def get_sound_horizon_rs_Mpc(H0, Omega_m0, Omega_b0):
    # ... (function body from previous turn is correct) ...
