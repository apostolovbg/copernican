...
def _context(tmp_path: Path) -> RuleContext:
    ...
    return RuleContext(repo_root=tmp_path, spec=spec, scope="repo", mode="full")


def test_full_suite_rule_requires_ci_pytest_and_policy_doc(tmp_path: Path) -> None:
    ...
    policy_doc = tmp_path / "DRIFTGUARD.md"
    policy_doc.write_text("Policy doc without test reminder.\n", encoding="utf-8")
  ...
  tests:
    steps:
      ...
      - name: Run DriftGuard policy check
        run: python -m driftguard.cli check --scope=repo --mode=full --repo-root .
""",
        encoding="utf-8",
    )
    policy_doc = tmp_path / "DRIFTGUARD.md"
    policy_doc.write_text(
        "Run the full program unit test suite in /tests before every commit and every "
        "task using both python -m pytest -q and python -m unittest discover -v.\n",
        encoding="utf-8",
    )
...
def test_full_suite_rule_requires_driftguard_after_tests(tmp_path: Path) -> None:
...
""",
    ...
    assert any("after the Pytest and Unit tests" in v.message for v in violations)
