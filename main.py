...
def main(argv=None):
    ...
    model_ast = load_model(
        model_path if model_path != 'test' else 'models/cosmo_model_lcdm.md')

    if hasattr(engine_module, 'Engine'):
        # New-style engine with a simple class interface
        engine = engine_module.Engine(model_ast)
        print(engine.run(model_ast, data_files, covar_file))
    else:
        # Legacy engines do not implement the new interface
        print('Selected engine is incompatible with the v2.0 CLI.')
        print('Please choose an engine that provides an Engine class.')
