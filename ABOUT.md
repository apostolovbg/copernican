...
**Version:** 10.9.6 **License:** See `LICENSE.md` (Copernican Suite developers,
2025)
...
dedicated Python 3.11 interpreter, isolates every dependency inside
`.venv`, and ships a GUI shell that mirrors the CLI workflow so analysts
can rapidly compare models, engines and datasets while keeping results
reproducible.
...
- **Dataset parity.** SNe, BAO and CMB sources live under
  `copernican/datasets/` with metadata, parsers and SHA256 digests; the
  registry enforces `dataset_id` alignment so every run manifest names the
  exact files used.
...
- **Live monitoring improvements.** The new Run Monitor navigation button
  ...
  sessions mimic the CLI exit routine while the diagnostics pane still provides
  View/Open/Flush buttons for its log stream.
...
- Launch `./.venv/bin/python -m copernican --gui` once the managed
  environment is ready, then use the Run Monitor to watch the live sampler
  progress.
