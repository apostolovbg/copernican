...
- Every game lives inside `rng_minigames/<id>/` with a `metadata.json`
  descriptor and a launcher module.
- `registry.json` caches hashes for every descriptor/launcher pair. Call
  `rng_minigames.refresh_registry()` after adding or editing a game so hosts
  can verify integrity before loading it.
...
- `rng_minigames.load_launcher(game_id)` automatically reloads the module each
  time it is invoked, making iterative development as simple as editing the
  file and reopening the mini-game window. No GUI restart is required.
...
- Each mini-game folder provides its own README with gameplay and configuration
  details.
- RNG-specific history is recorded in `rng_minigames/CHANGELOG.md`.
...
4. Load a launcher via `rng_minigames.load_launcher(descriptor.game_id)` and
   hand the context to it.
