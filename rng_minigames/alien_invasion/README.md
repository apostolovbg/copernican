...
future Earth by piloting a hovercraft across the bottom of the screen, dodging
barrages from four lieutenant rows, a shielded line of majors, the heavy
colonels and a perpetually evasive general. Every destroyed ship records its
identifier, and the finalized kill order plus the run duration are hashed into
...
fast-learning behavior (high learning speed, steeper kill rewards, softer
respawn penalties, and a heavy edge penalty) so the pilot quickly becomes a
challenging, human-beating opponent. Document every adjustment in
