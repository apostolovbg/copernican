...
def _ensure_number(value: object, field: str) -> float:
    """Return ``value`` as a float or raise :class:`PriorError`.
...
def prior_from_mapping(
    ...
    missing priors as uninformative.  The function normalises aliases such as
    ``type``/``distribution`` and raises :class:`PriorError` for incomplete
    specifications.
    """

    if not mapping:
        return None
    prior_type = mapping.get("type") or mapping.get("distribution")
    if not prior_type:
