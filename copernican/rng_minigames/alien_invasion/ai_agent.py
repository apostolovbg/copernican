...
class AlienInvasionAI:
    ...
    def decide(self, snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        shoot_roll = random.random()  # nosec B311 - AI RNG
        shoot = shoot_prob > shoot_roll

        charge_bias = weights["charge"] * 0.2 + urgency * 0.45
        charge_prob = min(0.98, max(0.01, forward["charge"] + charge_bias))
        charge_roll = random.random()  # nosec B311 - AI RNG
        charge = snapshot.get("charges", 0) > 0 and charge_prob > charge_roll

        exploration_roll = random.random()  # nosec B311 - AI RNG
        if exploration_roll < self.exploration_rate:  # exploration
            move_dir = random.choice([-1.0, 0.0, 1.0])  # nosec B311 - AI RNG
        shoot_flip_roll = random.random()  # nosec B311 - AI RNG
        if shoot_flip_roll < self.exploration_rate:
            shoot = not shoot
        charge_flip_roll = random.random()  # nosec B311 - AI RNG
        if charge_flip_roll < self.exploration_rate * 0.75:
            charge = not charge
