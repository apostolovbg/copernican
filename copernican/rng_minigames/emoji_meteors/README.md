...
on each sprite. The exact order of the pets, the time you needed to collect
the set and the emoji identities form the payload that is hashed into the
final seed. Because the mini-game runs inside the deterministic
`copernican/rng_minigames/` framework, two identical play sessions always
...
For high-level embedding guidance see `copernican/rng_minigames/README.md`;
the minigame loads on demand whenever a host calls
`load_launcher("emoji_meteors")`.
