...
def _validate_cmb_contract_definition_impl(
    ...
    from .likelihoods.cmb.runtime.planner import plan_cmb_numerics
    from .perturbation_contract import compile_perturbation_contract

    # Validate against an engine-generated numerical section.  Numerical
    # controls are not part of a model declaration, but hierarchy materializer
    # validation still needs a concrete momentum-grid shape.
    validation_perturbations = copy.deepcopy(dict(perturbations))
    validation_plan = plan_cmb_numerics(contract)
    validation_numerics = dict(validation_plan.numerical_controls)
    validation_numerics.update(validation_plan.hierarchy_controls)
    if validation_plan.momentum_grid_controls:
        validation_numerics["momentum_grids"] = {
            str(name): dict(values)
            for name, values in validation_plan.momentum_grid_controls.items()
        }
    validation_perturbations.pop("numerics", None)
    validation_perturbations.pop("accuracy_controls", None)
    validation_perturbations["numerics"] = validation_numerics
...
class ModelPlugin:
    ...
    def get_cmb_contract(self, values: Sequence[float]) -> dict[str, Any]:
        ...
        # Solver controls are generated by the CCMBS planner at execution
        # time; declarations never expose a model-owned numerical section.
        evaluated.pop("numerical", None)
