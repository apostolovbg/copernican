...
before forming residuals, matching the compressed UNITY convention without
changing the stored distance vector.
*Status:* The release includes the UNITY bookkeeping (Stan models, helper
utilities, `read_and_sample.py`, all inputs and the UNITY tarball), but the
CLI and GUI only consume the compressed µ/cov matrix today. When the FITS
file is updated rerunning the UNITY steps remains an option; the README
...
covariance matrices are assembled into a block-diagonal structure,
...
reads the full matrix, transforms it to \(D_\ell\) units and inverts it
