...
2. **Manifest generation**.
   ...
   engine-adapter metadata, model selections, and Git state for every run. The
   result is intentionally identical for CLI and GUI launches.
...
4. **Run result writing**.
   `copernican.lib.result_writer.save_summary` serialises sampler outputs
   while the logging and dependency helpers keep runtime flags aligned with the
   CLI.
...
the pipeline while still letting the GUI stream logs or status updates.
...
The orchestration contract assumes the active managed environment is already
selected before launch. That keeps GUI and CLI execution aligned with the same
interpreter, the same dependency surface, and the same manifest semantics.

Forward-only remains the default. The retired staged menu and
`COPERNICAN_ENABLE_STAGED_MENU` no longer define any runtime branch, so CI and
local launches both exercise the same unified GUI/CLI pipeline.
