...
purposes, but the right math operator is `**`. Care should be taken to preserve
...
`unicode_symbols` table provides pretty Unicode equivalents for console output.
## Allowed Macros
...
`latex_utils.py` ships with exhaustive lookup tables covering every Latin
