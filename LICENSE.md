...
- **Engine**
  The core executable code and libraries (Python source, compiled modules, etc.) that perform model parsing, numeric computation, plotting, and CSV output and/or any other action.

- **Model Specifications**
  YAML files (e.g., `cosmo_model_*.yml`) that declaratively define cosmological models, priors, and pipeline parameters.

- **Analysis Outputs**
  Any files generated by running the Software on a Model Specification (plots, CSVs, logs, posterior chains, etc.).
...
Apostolov grants each User a worldwide, non-exclusive, royalty-free license to install, run, execute and experiment with the Software for non-commercial, private and public research purposes **within either**:
