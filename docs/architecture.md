...
## Component Layers
### Orchestration Layer
- `python -m copernican` – entry point that handles argument parsing
 (`--gui`, `--cli`, `--manifest`, `--output-dir`), dependency scanning, logging
 (including `faulthandler` and SIG handlers), and then delegates to
 `copernican.lib.run_executor.execute_run_from_manifest`.
- `copernican/lib/run_executor.py` – reads manifests, revalidates datasets
 and model adapters, sets up logging via `copernican.lib.logger`, and invokes
 `copernican.lib.run_pipeline`.
- `copernican/lib/run_pipeline.py` – shared pipeline for Stages 1–5,
...
- `output/` – each run writes to `copernican-run_<UTC>` directories
 containing logs, plots, NetCDF chains, and `run_manifest_*` files for
 reproducibility.
...
1. **Stage 1 seed selection** – accepts `COPERNICAN_SEED`, accepts 0, or
 ...
 manifest.
2. **Stage 2 prompts** – after loading the CMB dataset, the CLI asks for
 burn-in, production steps, walkers, and pool size with recommended defaults
