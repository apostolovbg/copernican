...
This backend minimizes a single global chi-squared value constructed from all
available datasets (SNe Ia, BAO and CMB).  The actual numerical functions used
for distance calculations, sound horizon, etc. come from the model plugin
generated by :mod:`scripts.model_coder`.  The engine itself remains agnostic of
the cosmological model and purely orchestrates the optimisation and predictions.
"""
