...
def _ensure_camb():
    """Safely import CAMB or raise a helpful error message."""
    global camb
    if camb is not None:
        return camb
    ...
    try:
        ...
        if result.returncode != 0:
            if "No module named" in result.stderr:
                raise RuntimeError(
                    "CAMB is not installed in this Python environment. "
                    "Install it with 'pip install camb' (avoid pipx)."
                )
            raise RuntimeError(
