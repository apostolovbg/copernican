...
wheels. The dependency check now runs in read-only mode: it lists missing
packages with emoji markers and instructs you to rerun the platform launcher
so the managed `.venv` can be rebuilt. Installation no longer occurs inside
`copernican.py`, keeping runtime startup lean while the `start.*` scripts
remain responsible for applying the lock file. The launchers still expose an
*automatic dependency installation* toggle for operators who want the menu to
reinstall the stack before every run. Regenerate both files together whenever
dependencies change so the suite and published wheels remain in sync. The
pre-commit hook provisions
