...
so newcomers can read them without leaving the application.

Command-line operators can also run a few maintenance helpers without starting
the GUI menus:

- `python copernican.py --catalogue-summary` prints the dataset, model and
  engine inventory health summary.
- `python copernican.py --revalidate-dataset <dataset_id>` re-runs the parser
  trust check for the specified dataset and reports the digest result.
...
26. [Mini-game Seed Tools](rng_minigames/README.md)
...
- **Manifest entrypoint alignment** – the `copernican` console script registers
  `copernican.main_workflow`, which now passes the manifest to
  `copernican_lib.run_executor.execute_run_from_manifest`, keeping CLI and GUI
  launches on the same pipeline.
