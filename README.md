...
* `copernican_lib/` houses the reusable infrastructure—data loaders, numerical
  ...
  keep every engine and plugin consistent. Version 7.4.2 keeps the Stage 5
  corner plot responsive by deriving thinning statistics when legacy
  validators provide only samples and labels so archival plugins remain
  compatible while the downsampled visuals stay within memory limits.
