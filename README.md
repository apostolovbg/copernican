...
1. Ensure Python 3.12 or later is available. The suite requires `numpy`, `scipy`,
  ...
  program will advise reinstalling CAMB from source and exit cleanly. Ensure
  CAMB is installed using `pip` in the same environment; tools like `pipx` will
  install it elsewhere and the suite will not find it.
...
Make sure CAMB is installed in the **same Python environment** that runs the suite;
using `pipx` places it in a separate virtual environment which the program cannot
see.
