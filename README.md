...
* `copernican.py` presents the command-line experience, guiding users through
  dataset selection, model pairing and engine configuration. Build 7.6.6 keeps
  ...
  with the rest of the suite and preserves a fixed gap beneath the axes so
  exported figures remain consistent across dimensionalities.
...
* `copernican_lib/` houses the reusable infrastructure—data loaders, numerical
  ...
  keep every engine and plugin consistent. Version 7.6.6 documents the shared
  0.015 footer cadence used across Stage 5, recaps the live progress
