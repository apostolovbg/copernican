...
9. Each parameter may define a `prior` block describing sampling assumptions.
   ...
   `lower`/`upper` bounds. Legacy models may use `distribution` instead of
   `type`; both aliases are accepted and normalised. Log-uniform priors
...
they do not match the selected prior and inserting `transform: log` entries
whenever a log-uniform prior is declared.  This keeps the cached YAML
