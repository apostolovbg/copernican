...
* `copernican.py` presents the command-line experience, guiding users through
  dataset selection, model pairing and engine configuration. Build 7.7.4 keeps
  the nested-sampling backend pinned to one console row by switching the shared
  progress helper to carriage-return endings, eliminating the blank spacers that
  previously accumulated in long logs while preserving the spinner animation and
  iteration terminology. Build 7.7.3 keeps the nested-sampling backend on a
